...
*/
    ngAfterViewInit(): void {
        if (this._sort && this._paginator) {
            ...
            // Get stores if sort or page changes
            merge(this._sort.sortChange, this._paginator.page).pipe(
                switchMap(() => {
                    this.isLoading = true;
                    return this._storeService.getStores(this._paginator.pageIndex, this._paginator.pageSize, this.searchInputControl.value, this.status);
                }),
                map(() => {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    this.query = query;
                    this.isLoading = true;
                    return this._storeService.getStores(0, this._paginator.pageSize, query, this.status);
                }),
                map(() => {
                    this.isLoading = false;
                })
    ...
    onStoreClick(storeId: string) {
        this.router.navigate(['/stores', storeId]).then(() => {
            this._dialog.open(StoreDetailComponent, {
                width: '1080px',
                autoFocus: false
    ...
    statusSelectionChange(event: any) {
        ...
        this._storeService.getStores(0, this._paginator.pageSize, this.query, this.status).subscribe();
    }
}